---
# generated by https://github.com/hashicorp/terraform-plugin-docs
page_title: "infoblox_record_rpz_a List Resource - terraform-provider-infoblox"
subcategory: "RPZ"
description: |-
  Retrieves a list of Infoblox RecordRpzA from the NIOS backend.
---

# infoblox_record_rpz_a (List Resource)

Retrieves a list of Infoblox RecordRpzA from the NIOS backend.

## Example Usage

### NIOS Backend

```terraform
// List specific Substitute (IPv4 Address) Rules using filters
list "infoblox_record_rpz_a" "list_record_rpz_a_using_filters" {
  provider = infoblox
  config {
    filters = {
      name = "a.rpz.example.com"
    }
  }
}

// List specific Substitute (IPv4 Address) Rules using Extensible Attributes
list "infoblox_record_rpz_a" "list_record_rpz_a_using_extensible_attributes" {
  provider = infoblox
  config {
    ext_attr_filters = {
      Site = "location-1"
    }
  }
}

// List Substitute (IPv4 Address) Rules with resource details included
list "infoblox_record_rpz_a" "list_record_rpz_a_with_resource" {
  provider         = infoblox
  include_resource = true
}
```


<!-- schema generated by tfplugindocs -->
## Schema

### Optional

- `ext_attr_filters` (Map of String) Extensible Attribute Filters are used to filter results by NIOS extensible attributes. Only applicable for the NIOS backend.
- `filters` (Map of String) Filters are used to return a more specific list of results. Filters can be used to match resources by specific attributes (e.g. name, view). If multiple filters are specified, only resources that match all of them are returned.
