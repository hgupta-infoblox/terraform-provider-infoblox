...
var (
	_ list.ListResource                   = &LdapAuthServiceList{}
	_ list.ListResourceWithConfigure      = &LdapAuthServiceList{}
	_ list.ListResourceWithValidateConfig = &LdapAuthServiceList{}
)

func NewLdapAuthServiceList() list.ListResource {
	return &LdapAuthServiceList{}
}

type LdapAuthServiceList struct {
	backend core.BackendType
	service coresvc.LdapAuthServiceService
}

type LdapAuthServiceListModel struct {
	Filters types.Map `tfsdk:"filters"`
}

func (l *LdapAuthServiceList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_ldap_auth_service"
}

func (l *LdapAuthServiceList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewLdapAuthServiceService(l.backend, client.NIOS, client.UDDI)
}

func (l *LdapAuthServiceList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox LdapAuthService from the NIOS backend.",
...
func (l *LdapAuthServiceList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data LdapAuthServiceListModel
...
func (l *LdapAuthServiceList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data LdapAuthServiceListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_ldap_auth_service list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.LdapAuthService
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.LdapAuthService, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.LdapAuthService, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list LdapAuthService records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
