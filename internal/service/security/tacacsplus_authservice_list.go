...
var (
	_ list.ListResource                   = &TacacsplusAuthserviceList{}
	_ list.ListResourceWithConfigure      = &TacacsplusAuthserviceList{}
	_ list.ListResourceWithValidateConfig = &TacacsplusAuthserviceList{}
)

func NewTacacsplusAuthserviceList() list.ListResource {
	return &TacacsplusAuthserviceList{}
}

type TacacsplusAuthserviceList struct {
	backend core.BackendType
	service coresvc.TacacsplusAuthserviceService
}

type TacacsplusAuthserviceListModel struct {
	Filters types.Map `tfsdk:"filters"`
}

func (l *TacacsplusAuthserviceList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_tacacsplus_authservice"
}

func (l *TacacsplusAuthserviceList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewTacacsplusAuthserviceService(l.backend, client.NIOS, client.UDDI)
}

func (l *TacacsplusAuthserviceList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox TacacsplusAuthservice from the NIOS backend.",
...
func (l *TacacsplusAuthserviceList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data TacacsplusAuthserviceListModel
...
func (l *TacacsplusAuthserviceList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data TacacsplusAuthserviceListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_tacacsplus_authservice list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.TacacsplusAuthservice
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.TacacsplusAuthservice, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.TacacsplusAuthservice, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list TacacsplusAuthservice records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
