...
var (
	_ list.ListResource                   = &RadiusAuthserviceList{}
	_ list.ListResourceWithConfigure      = &RadiusAuthserviceList{}
	_ list.ListResourceWithValidateConfig = &RadiusAuthserviceList{}
)

func NewRadiusAuthserviceList() list.ListResource {
	return &RadiusAuthserviceList{}
}

type RadiusAuthserviceList struct {
	backend core.BackendType
	service coresvc.RadiusAuthserviceService
}

type RadiusAuthserviceListModel struct {
	Filters types.Map `tfsdk:"filters"`
}

func (l *RadiusAuthserviceList) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_radius_authservice"
}

func (l *RadiusAuthserviceList) Configure(_ context.Context, req resource.ConfigureRequest, resp *resource.ConfigureResponse) {
	...
	l.service = coresvc.NewRadiusAuthserviceService(l.backend, client.NIOS, client.UDDI)
}

func (l *RadiusAuthserviceList) ListResourceConfigSchema(_ context.Context, _ list.ListResourceSchemaRequest, resp *list.ListResourceSchemaResponse) {
	resp.Schema = listschema.Schema{
		MarkdownDescription: "Retrieves a list of Infoblox RadiusAuthservice from the NIOS backend.",
...
func (l *RadiusAuthserviceList) ValidateListResourceConfig(ctx context.Context, req list.ValidateConfigRequest, resp *list.ValidateConfigResponse) {
	var data RadiusAuthserviceListModel
...
func (l *RadiusAuthserviceList) List(ctx context.Context, req list.ListRequest, stream *list.ListResultsStream) {
	var data RadiusAuthserviceListModel
	...
	tflog.Info(ctx, fmt.Sprintf("infoblox_radius_authservice list: req.Limit=%d backend=%s includeResource=%t",
	...
	var records []*coremodel.RadiusAuthservice
	var err error
	...
	case core.BackendNIOS:
		records, err = core.ReadAllPagesNIOS(
			func(pageID string) ([]*coremodel.RadiusAuthservice, string, error) {
	...
	case core.BackendUDDI:
		records, err = core.ReadAllPagesUDDI(
			func(offset, _ int32) ([]*coremodel.RadiusAuthservice, error) {
	...
	if err != nil {
		diags.AddError("Client Error", fmt.Sprintf("Unable to list RadiusAuthservice records: %s", err))
		stream.Results = list.ListResultsStreamDiagnostics(diags)
		return
	}
